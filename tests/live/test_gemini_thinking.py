...
def test_sync(model, stream: bool = False, use_tool: bool = False):
    client = create_chat_client(BackendType.Gemini, model=model)
    if use_tool:
    ...
    if not stream:
        response = client.create_completion(
            messages=format_messages(messages, backend=BackendType.Gemini),
            stream=False,
            tools=tools_params,
            skip_cutoff=True,
            max_tokens=2000,
...
async def test_async(model, stream: bool = False, use_tool: bool = False):
    client = create_async_chat_client(BackendType.Gemini, model=model)
    if use_tool:
        messages = messages_for_tools_simple
        tools_params = tools_simple
    else:
...
if __name__ == "__main__":
    presets = {
        ...
        "gemini-2.5-flash": (BackendType.Gemini, "gemini-2.5-flash"),
    }
    backend, model = resolve_backend_model(BackendType.Gemini, "gemini-3-pro-preview", presets=presets)
    if backend != BackendType.Gemini:
        raise ValueError("test_gemini_thinking only supports BackendType.Gemini.")
