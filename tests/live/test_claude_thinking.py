...
def test_sync(model, stream: bool = False, use_tool: bool = False):
    client = create_chat_client(BackendType.Anthropic, model=model)
    if use_tool:
    ...
    if not stream:
        response = client.create_completion(
            messages=format_messages(messages, backend=BackendType.Anthropic),
            stream=False,
            tools=tools_params,
            skip_cutoff=True,
            max_tokens=2000,
...
async def test_async(model, stream: bool = False, use_tool: bool = False):
    client = create_async_chat_client(BackendType.Anthropic, model=model)
    if use_tool:
        messages = messages_for_tools_simple
        tools_params = tools_simple
    else:
...
if __name__ == "__main__":
    ...
    if backend != BackendType.Anthropic:
        raise ValueError("test_claude_thinking only supports BackendType.Anthropic.")
